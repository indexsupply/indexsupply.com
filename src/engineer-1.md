...
The current tech stack includes: Rust and Postgres deployed to EC2 Linux servers. Perhaps we will move to higher performance, lower cost providers in the future. I manage Postgres directly on the server and tune Linux and Postgres for the workload. The end result is a single (although multiple are run for redundancy) server (per chain) that is capable of serving 10,000 connected clients. I’ve been building with Postgres for over a decade and I know a lot of tricks to make it hum. If you have ever wanted to get knee-deep in Postgres (i.e. shared buffers, huge pages, b-tree vs. hash, nested loop joins vs. hash joins, etc.), or other low level systems, then you will likely enjoy this job!
